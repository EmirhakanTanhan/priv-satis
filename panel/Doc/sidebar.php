<div class="vb__menuLeft">
    <!--<div class="vb__menuLeft__mobileTrigger"><span></span></div>
    <div class="vb__menuLeft__trigger"></div>-->
    <div class="vb__menuLeft__outer">
        <div class="vb__menuLeft__logo__container">
            <div class="vb__menuLeft__logo">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" version="1.1" height="24px"
                     width="24px">
                    <g>
                        <path fill="#4b7cf3" strokeWidth="1" stroke="#4b7cf3"
                              d="M12,10.9c-0.1,0-0.2,0-0.2-0.1L3.5,6.1C3.4,6,3.3,5.8,3.3,5.6c0-0.2,0.1-0.3,0.2-0.4l8.2-4.7c0.2-0.1,0.3-0.1,0.5,0      l8.2,4.7c0.2,0.1,0.2,0.3,0.2,0.4S20.6,6,20.5,6.1l-8.2,4.7C12.2,10.8,12.1,10.9,12,10.9z M4.8,5.6L12,9.8l7.2-4.2L12,1.5      L4.8,5.6z"/>
                        <path fill="#4b7cf3" strokeWidth="1" stroke="#4b7cf3"
                              d="M13.6,23.6c-0.1,0-0.2,0-0.2-0.1c-0.2-0.1-0.2-0.3-0.2-0.4v-9.5c0-0.2,0.1-0.3,0.2-0.4l8.2-4.7c0.2-0.1,0.3-0.1,0.5,0      c0.2,0.1,0.2,0.3,0.2,0.4v9.5c0,0.2-0.1,0.3-0.3,0.4l-8.2,4.7C13.8,23.6,13.7,23.6,13.6,23.6z M14.1,13.9v8.3l7.2-4.2V9.8      L14.1,13.9z"/>
                        <path fill="#4b7cf3" strokeWidth="1" stroke="#4b7cf3"
                              d="M10.4,23.6c-0.1,0-0.2,0-0.2-0.1l-8.2-4.7c-0.2-0.1-0.3-0.3-0.3-0.4V8.9c0-0.2,0.1-0.3,0.2-0.4c0.2-0.1,0.3-0.1,0.5,0      l8.2,4.7c0.2,0.1,0.2,0.3,0.2,0.4v9.5c0,0.2-0.1,0.3-0.2,0.4C10.5,23.6,10.5,23.6,10.4,23.6z M2.7,18.1l7.2,4.2v-8.3L2.7,9.8      V18.1z"/>
                    </g>
                </svg>
                <div class="vb__menuLeft__logo__name">Visual Builder</div>
                <div class="vb__menuLeft__logo__descr">Html</div>
            </div>
        </div>
        <div class="vb__menuLeft__scroll vb__customScroll">
            <ul class="vb__menuLeft__navigation">

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel">
                        <span class="vb__menuLeft__item__title">Panel</span>
                        <i class="vb__menuLeft__item__icon fe fe-home"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/general">
                        <span class="vb__menuLeft__item__title">Genel Ayarlar</span>
                        <i class="vb__menuLeft__item__icon fe fe-settings"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/contents">
                        <span class="vb__menuLeft__item__title">İçerik Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-edit"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/menus">
                        <span class="vb__menuLeft__item__title">Menü Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-menu"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/banner">
                        <span class="vb__menuLeft__item__title">Banner Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-image"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/orders">
                        <span class="vb__menuLeft__item__title">Siparişler</span>
                        <i class="vb__menuLeft__item__icon fe fe-shopping-cart"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/products">
                        <span class="vb__menuLeft__item__title">Ürün Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-package"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/stocks">
                        <span class="vb__menuLeft__item__title">Stok Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-archive"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/category">
                        <span class="vb__menuLeft__item__title">Kategoriler</span>
                        <i class="vb__menuLeft__item__icon fe fe-tag"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/payment">
                        <span class="vb__menuLeft__item__title">Ödeme Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-credit-card"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/users">
                        <span class="vb__menuLeft__item__title">Üyeler</span>
                        <i class="vb__menuLeft__item__icon fe fe-users"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/blog">
                        <span class="vb__menuLeft__item__title">Blog</span>
                        <i class="vb__menuLeft__item__icon fe fe-bold"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/support/1">
                        <span class="vb__menuLeft__item__title">Destek</span>
                        <i class="vb__menuLeft__item__icon fe fe-life-buoy"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" target="_blank" href="/panel/storage/index.php">
                        <span class="vb__menuLeft__item__title">Depo Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-folder"></i>
                    </a>
                </li>

                <li class="vb__menuLeft__item ">
                    <a class="vb__menuLeft__item__link" href="/panel/logs">
                        <span class="vb__menuLeft__item__title">Log Yönetimi</span>
                        <i class="vb__menuLeft__item__icon fe fe-clipboard"></i>
                    </a>
                </li>


            </ul>

        </div>
    </div>
</div>